...
class BlockDiagonal(nn.Module):
    ...
    def forward(self, x):
        ...
        x = [block(x_i.to(next(self.parameters()).device)) for block, x_i in zip(self.blocks, x)]
        x = torch.cat(x, dim=-1)
        return x
...
class sLSTM(nn.Module):
    # TODO0: Add bias, dropout, bidirectional
...
class xLSTM(nn.Module):
    ...
    def forward(self, x, state=None):
        ...
        if state is not None:
            ...
            state = torch.zeros(self.num_layers, 4, batch_size, self.hidden_size).to(next(self.parameters()).device)
